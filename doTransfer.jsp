...
String n = Bank.getNameByNo("SELECT uname FROM acc_details WHERE acc_no = "+daccno);
String bal_sql_1 = "SELECT balance FROM tx_details WHERE uname='"+n+"' AND acc_no = "+daccno+" AND isnew = 'YES'";
int bal_2 = Bank.getBalance(bal_sql_1);
...
if(bal < amt ){
	...
	String sql3 = "INSERT INTO tx_details (uname, acc_no, operation, amt, balance, time, isnew ) VALUES ('"+uname+"',"+accno+",'WITHDRAW',"+amt+","+bal1+", '"+date+"', 'YES')";
	int update = Bank.update(sql3);
	//deposite into other account
	Bank.update("UPDATE tx_details SET isnew = 'NO' WHERE uname='"+n+"' AND acc_no = "+daccno+" AND isnew = 'YES'");
	bal_2 = bal_2 + amt;
	String sql = "INSERT INTO tx_details (uname, acc_no, operation, amt, balance, time, isnew ) VALUES ('"+n+"',"+daccno+",'DEPOSITE',"+amt+","+bal_2+", '"+date+"', 'YES' )";
	int u = Bank.update(sql);
